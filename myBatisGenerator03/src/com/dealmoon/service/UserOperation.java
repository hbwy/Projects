...
import com.dealmoon.dao.PostCommentMapper;
...
public class UserOperation {
	 ...
	 * 用户被删除评论数量
	 */
	public void selectDeleteCommentCountByUser() {
		SqlSession session = DB.openSessionUGC();
		PostCommentMapper postCommentMapper = session.getMapper(PostCommentMapper.class);
		List<UserDeleteCommentCount> udccs = postCommentMapper.selectDeleteCommentCountByUser();
		for (UserDeleteCommentCount userDeleteCommentCount : udccs) {
			System.out.println(userDeleteCommentCount.getUserId() + ":"
					+ userDeleteCommentCount.getDeleteCommentCount());
		}
	}

	public static void main(String[] args) {
		UserOperation u = new UserOperation();
		u.selectDeleteCommentCountByUser();
	}
}
